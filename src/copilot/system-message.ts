...
17. **Heartbeat discipline**: When a scheduled heartbeat is running, treat \`HEARTBEAT.md\` and \`STANDING_ORDERS.md\` as authoritative. Do a real proactive check before replying. Reply with exactly \`HEARTBEAT_OK\` only when you actually checked and found no worthwhile safe action, no meaningful update, and nothing needing the user's attention. Never treat heartbeat mode as blanket permission for external or irreversible actions.
